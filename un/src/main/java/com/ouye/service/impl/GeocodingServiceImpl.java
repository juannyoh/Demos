...
//				}
	...
	public Map<String, Object> getCountyByAdmincode(String admincode) {
		...
		if(admincode.length() < 6){
			...
			qp.setAttributeFilter(filter);
			qp.setGroupBy(new String[]{"COUNTY"});
			qp.setOrderBy(new String[]{"ADMINCODE_MZ"});
		}else{
	...
	public String searchAdmincodeByName(String provicename,String cityname,String countyname,String townname, int level) {
		...
		if(level==1){
			...
			dv=searchByUDB.getDatasectByName(config.getDatasetNameTown());//乡镇数据集
		}
		QueryParameter qp = new QueryParameter();
		String filter = null;
		if(level==1){
		...
		if(null != recordset && recordset.getRecordCount() > 0){
			...
			if(level==1||level==2||level==3){
				admincode= recordset.getString("ADMINCODE");
			}
			else{
				admincode= recordset.getString("ADMINCODE_MZ");
			}
		}
		if(null!=recordset){
			recordset.close();
